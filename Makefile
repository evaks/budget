...
output/server.min.js: node_modules $(wildcard plugins/**/*.server.js)  $(wildcard plugins/**/*.sql) $(wildcard plugins/**/*.shared.js) $(wildcard aurora/plugins/**/*.server.js) $(shell find aurora/plugins -name '*.shared.js') $(shell find plugins/recoil -name '*.js' -print) $(shell find plugins/closure-library -name '*.js') $(wildcard plugins/**/*.schema.json) $(wildcard plugins/**/genschema.js)
...
upgrade:
	...
	sudo `which node` /var/www/server.min.js --upgrade
	sudo service budget start
