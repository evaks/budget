...
budget.widgets.Budget.createMovableSizable = function(tableB) {
    //
    let frp = tableB.frp();
    ...
    return frp.liftBI(function(tbl, decorators) {
        let hasHeaderRow = !!tbl.getMeta().headerRowDecorator;
        if (tbl.size() === 0 && !hasHeaderRow) {
            ...
            tbl.forEachPlacedColumn(function(col, meta) {
                res.addColumnMeta({name: 'bob'});
            });
            res.addMeta({headerRowDecorator: emptyHeaderRowDecorator});
            console.log('making dummy header row');
            return res.freeze();
        }
        let res = tbl.createEmpty([], [ADD_COL, DEL_COL, MOVE_COL]);
        let lastPos = 0;
        ...
        res.addColumnMeta(DEL_COL, {position: lastPos + 2, cellWidgetFactory: delFactory});
